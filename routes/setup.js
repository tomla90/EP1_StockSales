...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if (importResult === "itemsExist") {
      res.jsend.fail({
        message:
          "Items already exist in the database. Import and Admin creation aborted.",
      });
    } else if (importResult === "invalidData") {
      res.jsend.fail({
        message: "Received data is not an array. Import aborted.",
      });
      ...
      try {
        ...
        res.jsend.success({
          message: "Items imported successfully and admin user created.",
        });
      } catch (error) {
